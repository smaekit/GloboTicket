...
namespace GloboTicket.Services.Ordering.Messaging
{
    public class AzServiceBusConsumer: IAzServiceBusConsumer
    {
        ...
        public AzServiceBusConsumer(IConfiguration configuration, IMessageBus messageBus, OrderRepository orderRepository)
        {
            ...
            {
                Console.WriteLine("WARNING: Azure Service Bus not configured. Async messaging disabled.");
            }
        }

        public void Start()
        {
            if (checkoutMessageReceiverClient == null || orderPaymentUpdateMessageReceiverClient == null)
            {
                Console.WriteLine("WARNING: Service Bus consumers not initialized. Skipping message registration.");
                return;
            }
